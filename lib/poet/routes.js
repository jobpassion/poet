...
function bindRoutes (poet) {
  ...
  app.post('/post/:post', postRouteGenerator(poet, 'post'))
}
...
function postRouteGenerator (poet, view) {
  return function postRoute (req, res, next) {
      //check is comment
      if("POST"==req.method){
        console.log('post a commment:' + req.body.comment);
        req.body.slug = req.params.post;
        utils.addComment(poet, poet.options, req.body);
      }
    var post = poet.helpers.getPost(req.params.post);
    if (post) {
      res.render(view, { post: post ,comments:poet.comments[req.params.post]});
    } else {
      next();
    }
  };
