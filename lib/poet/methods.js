...
//add by jeff
  ...
  poet.comments = {};
var pool = utils.createPool(options);
 var connection = pool.getConnection(function(err, connection) {
        var query = connection.query('select * from comments', function(err, result) {
            for(var i in result){
                if(!poet.comments[result[i].slug]){
                    poet.comments[result[i].slug] = [];
                }
                poet.comments[result[i].slug].push(result[i]);
            }
        });
